...
var YEAR_FILTER = 'after:2025/12/31 before:2027/01/01';
...
function processRegistrationEmails() {
  ...
  try {
    ...
    var query = 'subject:"' + EMAIL_SUBJECT + '" -label:' + PROCESSED_LABEL + ' ' + YEAR_FILTER;
    var threads = GmailApp.search(query);
    Logger.log('Found ' + threads.length + ' unprocessed 2026 thread(s)');
    ...
    for (var t = 0; t < threads.length; t++) {
      ...
      for (var m = 0; m < messages.length; m++) {
        ...
        // Skip messages not from 2026
        var msgDate = message.getDate();
        if (msgDate.getFullYear() !== 2026) {
          Logger.log('Skipping message from ' + msgDate.getFullYear() + ' (not 2026)');
          continue;
        }
...
function testWithLatestEmail() {
  var threads = GmailApp.search('subject:"' + EMAIL_SUBJECT + '" ' + YEAR_FILTER, 0, 1);
  if (threads.length === 0) {
    Logger.log('No 2026 registration emails found.');
    return;
  }
